function WriteText(Direc,Text,Mode) f = io.open(Direc, Mode) f:write(Text) f:close() end _mr = gg.makeRequest _wf = WriteText _gf = gg.getFile _pr = gg.alert _prt = print
...
_lcs = true
_da = false
chlcs = function() if #tf ~= 79725 then if _da == false then _a = _pr(lcs,clcs,rlsc) else _a = _pr(lcs,clcs,rlsc,dlcs) end if _a == 1 then _da = true end if _a == 2 then org = _mr("").content if not orgsc then _pr(err) else pcall(load(org)) _wr(_gf(), org, "w") _lcs = false end end if _a == 3 then _prt(lcs) _lcs = false end end end
